...
void cSelectTimer::Draw(const int X, const int Y) {
	int tX = 0, tY = -15;
	tX -= GetDrawFormatStringWidthToHandle(mBigFont, _T("%d"), mTime / static_cast<int>(REFRESH_RATE));
	tX -= GetDrawFormatStringWidthToHandle(mSmallFont, _T(".%d"), abs(mTime % static_cast<int>(REFRESH_RATE) / static_cast<int>(REFRESH_RATE / 10.0)));
	tX /= 2;
	...
	DrawFormatStringToHandle(tX + X, tY + Y, GetColor(0xFF, 0xFF, 0xFF), mBigFont, _T("%d"), mTime / static_cast<int>(REFRESH_RATE));
	DrawFormatStringToHandle(tX + X + GetDrawFormatStringWidthToHandle(mBigFont, _T("%d"), mTime / static_cast<int>(REFRESH_RATE)), tY + Y + 8, GetColor(0xFF, 0xFF, 0xFF), mSmallFont, _T(".%d"), abs(mTime % static_cast<int>(REFRESH_RATE) / static_cast<int>(REFRESH_RATE / 10.0)));
}