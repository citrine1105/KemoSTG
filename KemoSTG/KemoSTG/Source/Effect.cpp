...
void cEffect::Draw() {
	...
	default:
		if (gEffectImageContainer.GetElement(mType)->GetHandleNum() > 0) {
			DrawGraphF(mPosition.GetX(), mPosition.GetY(), gEffectImageContainer.GetElement(mType)->GetHandle(static_cast<int>(static_cast<double>(mAnimeTimer.GetTime()) / static_cast<double>(gEffectImageContainer.GetElement(mType)->GetHandleNum()))), TRUE);
		}
		break;
	}
}